...
{
	$sel_b="SELECT * from courseinfo where course_id=".$course_id;
	$res=mysql_query($sel_b) or die(mysql_error());
	$b_name=mysql_fetch_array($res);
	return $b_name['b_name'];
}

function student_name($stud_id)
{
	$sel_b="SELECT * from studentinfo where stud_id=".$stud_id;
	$res=mysql_query($sel_b) or die(mysql_error());
	$b_name=mysql_fetch_array($res);
	return $b_name['username'];
}

/*
function sem_name($sem_id)
{
	$sel_s="SELECT * from facultyinfo where sem_id=".$sem_id;
